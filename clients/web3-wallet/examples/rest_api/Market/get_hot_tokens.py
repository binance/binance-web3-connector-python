...
def get_hot_tokens():
    try:
        response = client.rest_api.get_hot_tokens()

        rate_limits = response.rate_limits
        logging.info(f"get_hot_tokens() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_hot_tokens() response: {data}")
    except Exception as e:
        logging.error(f"get_hot_tokens() error: {e}")
