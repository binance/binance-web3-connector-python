...
def get_token_basic_info():
    try:
        response = client.rest_api.get_token_basic_info(
            binance_chain_id="1",
            token_contract_address="0xdAC17F958D2ee523a2206206994597C13D831ec7",
        )

        rate_limits = response.rate_limits
        logging.info(f"get_token_basic_info() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_token_basic_info() response: {data}")
    except Exception as e:
        logging.error(f"get_token_basic_info() error: {e}")


if __name__ == "__main__":
    get_token_basic_info()
