...
def get_total_value_by_address():
    try:
        response = client.rest_api.get_total_value_by_address()

        rate_limits = response.rate_limits
        logging.info(f"get_total_value_by_address() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_total_value_by_address() response: {data}")
    except Exception as e:
        logging.error(f"get_total_value_by_address() error: {e}")


if __name__ == "__main__":
    get_total_value_by_address()
