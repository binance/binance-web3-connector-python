...
def get_investment_detail():
    try:
        response = client.rest_api.get_investment_detail(
            investment_id="investment_id_example",
        )

        rate_limits = response.rate_limits
        logging.info(f"get_investment_detail() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_investment_detail() response: {data}")
    except Exception as e:
        logging.error(f"get_investment_detail() error: {e}")
