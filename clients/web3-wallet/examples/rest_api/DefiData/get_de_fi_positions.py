...
def get_de_fi_positions():
    try:
        response = client.rest_api.get_de_fi_positions(
            addresses=[""],
        )

        rate_limits = response.rate_limits
        logging.info(f"get_de_fi_positions() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_de_fi_positions() response: {data}")
    except Exception as e:
        logging.error(f"get_de_fi_positions() error: {e}")
